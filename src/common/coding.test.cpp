...
TEST_CASE("varint::encode() 编码单字节值时应只输出一个字节", "[common][coding]")
{
    // 测试单字节编码（值 < 128）
...
TEST_CASE("varint::encode() 编码多字节值时应输出预期字节序列", "[common][coding]")
{
    // 测试多字节编码
    std::vector<std::byte> buffer;
...
TEST_CASE("varint::encode() 编码零值时应输出单个零字节", "[common][coding]")
{
    std::vector<std::byte> buffer;
...
TEST_CASE("varint::encode() 编码 uint8 最大值时应输出两个字节", "[common][coding]")
{
    std::vector<std::byte> buffer;
...
TEST_CASE("varint::encode() 编码 uint16 最大值时应输出三个字节", "[common][coding]")
{
    std::vector<std::byte> buffer;
...
TEST_CASE("varint::encode() 编码 uint32 最大值时应输出五个字节", "[common][coding]")
{
    std::vector<std::byte> buffer;
...
TEST_CASE("varint::encode() 编码 uint64 最大值时应输出十个字节", "[common][coding]")
{
    std::vector<std::byte> buffer;
...
TEST_CASE("varint::decode() 解码单字节时应返回正确值与迭代器位置", "[common][coding]")
{
...
TEST_CASE("varint::decode() 解码多字节时应返回正确值与迭代器位置", "[common][coding]")
{
...
TEST_CASE("varint::decode() 解码零值时应返回零", "[common][coding]")
{
...
TEST_CASE("varint::decode() 解码 uint8 最大值编码结果时应还原为 255", "[common][coding]")
{
...
TEST_CASE("varint uint8 全范围往返编解码时应保持数值一致", "[common][coding]")
{
...
TEST_CASE("varint uint16 边界值往返编解码时应保持数值一致", "[common][coding]")
{
...
TEST_CASE("varint uint32 边界值往返编解码时应保持数值一致", "[common][coding]")
{
...
TEST_CASE("varint uint64 边界值往返编解码时应保持数值一致", "[common][coding]")
{
...
TEST_CASE("varint::encode() 使用 char 迭代器时应输出正确字节", "[common][coding]")
{
...
TEST_CASE("varint::decode() 使用 char 迭代器时应返回正确值与迭代器位置", "[common][coding]")
{
    std::vector<char> buffer;
    varint::encode(std::back_inserter(buffer), uint16_t{300});
...
TEST_CASE("varint uint16 经 char 迭代器往返编解码时应保持数值一致", "[common][coding]")
{
...
TEST_CASE("varint::encode() 对不同数值范围应输出预期字节长度", "[common][coding]")
{
    // 单字节: 0 ~ 127
...
TEST_CASE("varint::encode() 连续编码时应按顺序写入字节流", "[common][coding]")
{
    std::vector<std::byte> buffer;
...
TEST_CASE("varint::decode() 连续解码时应按顺序恢复所有值", "[common][coding]")
{
    std::vector<std::byte> buffer;
...
TEST_CASE("varint::encode() 写入 std::string 时应输出正确字节", "[common][coding]")
{
    std::string buffer;
    varint::encode(std::back_inserter(buffer), uint32_t{42});
...
TEST_CASE("varint::decode() 从 std::string 读取时应返回正确值与迭代器位置", "[common][coding]")
{
    std::string buffer;
    varint::encode(std::back_inserter(buffer), uint16_t{300});
...
TEST_CASE("varint uint32 经 std::string 往返编解码时应保持数值一致", "[common][coding]")
{
...
TEST_CASE("varint 经 std::string 连续编解码时应按顺序完整恢复", "[common][coding]")
{
    std::string buffer;
...
TEST_CASE("varint 经 std::string 与 vector 编码同一数值时字节序列应一致", "[common][coding]")
{
...
TEST_CASE("varint uint32 随机值往返编解码时应保持数值一致", "[common][coding]")
{
    // 测试一些随机的 uint32 值
...
TEST_CASE("varint 混合类型连续编解码时应按顺序正确恢复", "[common][coding]")
{
    // 在同一个缓冲区中编码不同类型的值
    std::vector<std::byte> buffer;
...
TEST_CASE("varint 混合类型边界值编解码时应正确恢复各边界值", "[common][coding]")
{
    // 使用边界值测试混合类型
    std::vector<std::byte> buffer;
...
TEST_CASE("varint 经 std::string 混合类型编解码时应按顺序完整恢复", "[common][coding]")
{
...
TEST_CASE("pack 拼接 1 字节时应正确左移并合并", "[common][coding]")
{
    auto result = pack(0x12ULL, pack_bytes<1>(0x34U));
    REQUIRE(result == 0x1234ULL);
}

TEST_CASE("pack 连续拼接字节时应逐级正确组装", "[common][coding]")
{
...
TEST_CASE("pack 拼接时超出 N 字节的高位应被掩码剔除", "[common][coding]")
{
...
TEST_CASE("unpack 从拼接值提取首个字节时应正确解包", "[common][coding]")
{
...
TEST_CASE("unpack 连续解包字节时应逐级正确分离", "[common][coding]")
{
...
TEST_CASE("pack 后再 unpack 应恢复原始值", "[common][coding]")
{
...
TEST_CASE("pack 在最大 N 值场景下应正确处理", "[common][coding]")
{
...
TEST_CASE("pack 在 dest 为 0 时应正确处理", "[common][coding]")
{
    auto result = pack(0ULL, pack_bytes<2>(0xABCDU));
    REQUIRE(result == 0xABCDULL);
}

TEST_CASE("pack 在 dest 为全 1 时应正确处理", "[common][coding]")
{
...
TEST_CASE("pack 在 src 为全 0 时应正确处理", "[common][coding]")
{
    auto result = pack(0x1234ULL, pack_bytes<2>(0U));
    REQUIRE(result == 0x12340000ULL);
}

TEST_CASE("pack 在 src 为全 1 时应正确处理", "[common][coding]")
{
...
TEST_CASE("unpack 提取全 1 掩码时应正确处理", "[common][coding]")
{
...
TEST_CASE("unpack 提取全 0 值时应正确处理", "[common][coding]")
{
...
TEST_CASE("多次 pack 后再多次 unpack 应恢复原始值", "[common][coding]")
{
...
TEST_CASE("pack 应正确编码 32 位地址的段偏移（模拟段式内存寻址）", "[common][coding]")
{
...
TEST_CASE("pack 应正确编码多字节数据结构（模拟网络包头）", "[common][coding]")
{
...
TEST_CASE("unpack 应正确解析多字节编码数据", "[common][coding]")
{
    // 从网络包头中逐步解包
...
TEST_CASE("pack 处理大值 pack_bytes 到小类型时应正确掩码", "[common][coding]")
{
...
TEST_CASE("unpack 连续多次解包时应还原完整值", "[common][coding]")
{
...
TEST_CASE("pack 与 unpack 在最大 N 值下应正确处理 (N = sizeof(T) - 1)", "[common][coding]")
{
...
TEST_CASE("pack 应正确处理交替的不同字节宽度编码", "[common][coding]")
{
...
TEST_CASE("varint::length() 对单字节值应返回 0", "[common][coding]")
{
...
TEST_CASE("varint::length() 应与实际编码字节数一致", "[common][coding]")
{
...
TEST_CASE("fixed::encode() 应输出固定字节数", "[common][coding]")
{
    std::vector<std::byte> buf1;
...
TEST_CASE("fixed::encode() 应使用小端字节序", "[common][coding]")
{
    std::vector<std::byte> buffer;
...
TEST_CASE("fixed::encode() 对零值应输出零字节", "[common][coding]")
{
    std::vector<std::byte> buffer;
...
TEST_CASE("fixed::encode() 对最大值应输出全 1 字节", "[common][coding]")
{
    std::vector<std::byte> buffer;
...
TEST_CASE("fixed::decode() 应恢复编码后的值", "[common][coding]")
{
    std::vector<std::byte> buffer;
...
TEST_CASE("fixed::encode/decode() 往返后应保持值一致", "[common][coding]")
{
...
TEST_CASE("fixed::encode() 到 char 迭代器时应正确编码", "[common][coding]")
{
    std::string buffer;
...
TEST_CASE("fixed::decode() 从 char 迭代器读取时应正确解码", "[common][coding]")
{
    std::string buffer;
    const uint32_t original = 0x12345678;
    fixed::encode(std::back_inserter(buffer), original);
...
TEST_CASE("fixed::encode() 连续编码多个值时应按顺序写入", "[common][coding]")
{
    std::vector<std::byte> buffer;
...
TEST_CASE("fixed::decode() 连续解码多个值时应按顺序恢复", "[common][coding]")
{
    std::vector<std::byte> buffer;
...
TEST_CASE("fixed::encode() 与 varint::encode() 的编码大小应不同", "[common][coding]")
{
...
TEST_CASE("fixed::encode() 在大值场景下应比 varint 更紧凑", "[common][coding]")
{
    std::vector<std::byte> fixed_buf;
    std::vector<std::byte> varint_buf;
