...
TEST_CASE("在向容量为2的缓存插入单个元素后，应该能查询到对应值", "[lru_cache]")
{
...
TEST_CASE("在查询不存在的键时，应该返回空指针", "[lru_cache]")
{
    LRUCache<int, std::string> cache{2};
    cache.put(1, "one");
...
TEST_CASE("在容量内连续插入多个元素时，应该全部可查询", "[lru_cache]")
{
...
TEST_CASE("当插入元素超过容量时，应该驱逐最久未使用的元素", "[lru_cache]")
{
    LRUCache<int, std::string> cache{2};
    cache.put(1, "one");
    cache.put(2, "two");
...
TEST_CASE("在查询已存在键后再插入新元素时，应该驱逐最久未查询的元素", "[lru_cache]")
{
    LRUCache<int, std::string> cache{2};
    cache.put(1, "one");
    cache.put(2, "two");
...
TEST_CASE("在更新已存在的键时，应该返回更新后的值", "[lru_cache]")
{
    LRUCache<int, std::string> cache{2};
    cache.put(1, "one");
    cache.put(2, "two");
...
TEST_CASE("在更新键后再插入新元素时，应该驱逐旧的键", "[lru_cache]")
{
    LRUCache<int, std::string> cache{2};
    cache.put(1, "one");
    cache.put(2, "two");
...
TEST_CASE("在容量为1的缓存中频繁插入时，应该持续驱逐前一个元素", "[lru_cache]")
{
...
TEST_CASE("在使用 string 键和 int 值时，应该返回对应缓存值", "[lru_cache]")
{
...
TEST_CASE("在缓存中存储复杂对象时，应该取回完整对象内容", "[lru_cache]")
{
    struct Item {
        int id;
        std::string name;
...
TEST_CASE("在使用自定义内存资源插入新键时，应该通过该资源完成节点分配", "[lru_cache]")
{
...
TEST_CASE("在使用自定义内存资源命中已有键时，应该不新增节点分配", "[lru_cache]")
{
    TrackingMemoryResource resource{ std::pmr::new_delete_resource() };
...
TEST_CASE("在使用自定义内存资源的缓存析构后，应该释放内部节点占用", "[lru_cache]")
{
    TrackingMemoryResource resource{ std::pmr::new_delete_resource() };
