...
TEST_CASE("在写入单条记录时，应该生成 header 加 payload 的完整字节长度", "[leveldb][wal][writer]")
{
	TempLogFile temp;
...
TEST_CASE("Writer 写入空字符串时应产生空 payload 的 Full record", "[leveldb][wal][writer]")
{
	TempLogFile temp;
...
TEST_CASE("在写入单条记录时，应该将 record type 编码为 Full", "[leveldb][wal][writer]")
{
	TempLogFile temp;
	constexpr auto payload = "wal-entry";
...
TEST_CASE("在写入单条记录时，应该原样写入 payload 内容", "[leveldb][wal][writer]")
{
	TempLogFile temp;
	constexpr auto payload = "wal-entry";
...
TEST_CASE("在写入单条记录时，应该写入可校验通过的 checksum", "[leveldb][wal][writer]")
{
	TempLogFile temp;
	constexpr auto payload = "wal-entry";
...
TEST_CASE("在写入空字符串时，应该只生成 header 字节", "[leveldb][wal][writer]")
{
	TempLogFile temp;
...
TEST_CASE("在写入空字符串时，应该将 payload 长度编码为 0", "[leveldb][wal][writer]")
{
	TempLogFile temp;
...
TEST_CASE("在写入空字符串时，应该将 record type 编码为 Full", "[leveldb][wal][writer]")
{
	TempLogFile temp;
...
TEST_CASE("在写入空字符串时，应该写入可校验通过的 checksum", "[leveldb][wal][writer]")
{
	TempLogFile temp;
...
TEST_CASE("在写入超大记录时，应该拆分成多个 physical record", "[leveldb][wal][writer]")
{
	TempLogFile temp;
...
TEST_CASE("在写入超大记录时，应该将首分片类型编码为 First", "[leveldb][wal][writer]")
{
	TempLogFile temp;
	const auto payload = make_large_payload(200000);
...
TEST_CASE("在写入超大记录时，应该将尾分片类型编码为 Last", "[leveldb][wal][writer]")
{
	TempLogFile temp;
	const auto payload = make_large_payload(200000);
...
TEST_CASE("在写入超大记录时，应该将中间分片类型编码为 Middle", "[leveldb][wal][writer]")
{
	TempLogFile temp;
	const auto payload = make_large_payload(200000);
...
TEST_CASE("在写入超大记录时，应该可由分片 payload 重组出原始内容", "[leveldb][wal][writer]")
{
	TempLogFile temp;
	const auto payload = make_large_payload(200000);
...
TEST_CASE("在写入超大记录时，应该保证每个分片 checksum 都可校验通过", "[leveldb][wal][writer]")
{
	TempLogFile temp;
	const auto payload = make_large_payload(200000);
...
TEST_CASE("在读取单条日志记录时，应该返回对应的记录内容", "[leveldb][wal][reader]")
{
	TempLogFile temp;
...
TEST_CASE("在读取多条日志记录时，应该保持写入顺序", "[leveldb][wal][reader]")
{
	TempLogFile temp;
...
TEST_CASE("在读取分片日志记录时，应该重组出完整内容", "[leveldb][wal][reader]")
{
	TempLogFile temp;
	const auto payload = make_large_payload(200000);
...
TEST_CASE("在读取 checksum 损坏日志且 paranoid_checks 关闭时，应该返回空结果", "[leveldb][wal][reader]")
{
	TempLogFile temp;
...
TEST_CASE("在读取 checksum 损坏日志且 paranoid_checks 开启时，应该抛出 CorruptionException", "[leveldb][wal][reader]")
{
	TempLogFile temp;
	constexpr auto payload = "corrupted-record";
