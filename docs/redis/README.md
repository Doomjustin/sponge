...
- [Redis 命令整理（推荐命令）](#redis-命令整理推荐命令)
- [概述](#概述)
- [适合场景](#适合场景)
- [快速入口](#快速入口)
- [架构概览](#架构概览)
...
为了避免和本仓库当前实现混淆，Redis 推荐命令与选项语义单独整理在：

- [docs/redis/REDIS_COMMANDS.md](docs/redis/REDIS_COMMANDS.md)
...
- 有序集合类型：[include/sponge/redis/sorted_set.h](include/sponge/redis/sorted_set.h)
- AOF 持久化：[include/sponge/redis/aof.h](include/sponge/redis/aof.h)
...
- [include/sponge/redis/aof.h](include/sponge/redis/aof.h)
- [include/sponge/redis/application_context.h](include/sponge/redis/application_context.h)
- [include/sponge/redis/dash_table.h](include/sponge/redis/dash_table.h)
- [include/sponge/redis/db_shard.h](include/sponge/redis/db_shard.h)
...
- [include/sponge/redis/skip_list.h](include/sponge/redis/skip_list.h)
- [include/sponge/redis/sorted_set.h](include/sponge/redis/sorted_set.h)
- [include/sponge/redis/string.h](include/sponge/redis/string.h)
- [include/sponge/redis/ttl_manager.h](include/sponge/redis/ttl_manager.h)

从接口稳定性来看，当前对外较稳定的部分主要是：

- `AOF`：异步追加写日志，用于持久化
- `Server`：服务端入口
- `String`：Redis 风格动态字符串
- `ListPack`：Redis 风格紧凑列表容器
- `SortedSet`：有序集合，内部自动在 ListPack 和 SkipList 之间切换后端

而协议解析、命令派发等服务端内部能力则位于 [src/redis](src/redis) 下的内部头文件中。

## AOF（追加写日志）

见 [核心概念 → AOF（追加写日志）](#1-aof追加写日志)。

## SortedSet（有序集合）

`SortedSet` 是 Redis 风格的有序集合，按分数（`double`）对成员排序，基于 PMR 内存资源分配。内部会根据数据量自动选择后端编码：

- 数据量较小时使用 `ListPack`（默认阈值 128 个元素），紧凑连续存储
- 超过阈值后升级为 `SkipList` + 哈希字典（`boost::unordered_node_map`）双索引结构，支持 $O(\log n)$ 的按分数查找

相关入口如下：

- 接口定义：[include/sponge/redis/sorted_set.h](include/sponge/redis/sorted_set.h)
- 实现代码：[src/redis/sorted_set.cpp](src/redis/sorted_set.cpp)
- 单元测试：[src/redis/sorted_set.test.cpp](src/redis/sorted_set.test.cpp)

当前主要接口：

- `add(Score score, Member member)`：添加成员，若已存在则更新分数
- `score(Member member)`：查询指定成员的分数
...
`AOF`（Append-Only File）是 Redis 模块的持久化组件，负责把执行过的命令异步追加写入磁盘，作为故障恢复的依据。

相关入口如下：

- 接口定义：[include/sponge/redis/aof.h](include/sponge/redis/aof.h)
- 实现代码：[src/redis/aof.cpp](src/redis/aof.cpp)
- 单元测试：[src/redis/aof.test.cpp](src/redis/aof.test.cpp)

当前接口：

- `AOF(std::string_view filename)`：构造时打开 AOF 文件，启动后台 I/O 线程
- `append(std::string command)`：把写入任务异步投递到后台线程（立即返回，不阻塞调用方）
- `is_healthy()`：查询当前写入通道是否健康

**设计要点：**

AOF 内部持有一个单线程的 Boost.Asio `io_context`，外部调用 `append()` 只做一次 `boost::asio::post()`，本身完全不阻塞。后台线程负责实际写入：

- 写入成功后调用 `flush()`，确保数据刷出 `std::ofstream` 缓冲区
- 写入失败时进入等待-重试循环（每次间隔 1 秒），并通过原子变量 `healthy_` 记录故障状态
- 写入恢复后 `healthy_` 自动重置，并打印 `AOF is recovered` 日志

**关闭顺序（析构器）：**

```
1. work_guard_.reset()   — 允许 io_context 在队列耗尽后自然退出
2. io_context_.stop()    — 强制停止（丢弃队列中尚未执行的任务）
3. thread_.join()        — 等待后台线程退出
4. file_.sync()          — fsync：把所有内核缓冲区数据持久化到磁盘
```

> **注意**：第 2 步的 `io_context_.stop()` 会丢弃队列中还未执行的写入任务。如果在 AOF 对象销毁时仍有 `append()` 任务在排队，这些命令将**不会**写入磁盘。代码中注释掉的 `stopping_` 原本是为了在析构阶段拒绝新任务，目前未启用，需要注意这个潜在的数据丢失场景。
...
- aof
- application_context
- command / command_context / commands
- dash_table
- db_shard
- list_pack
- protocol
...
**加锁实现复测（随机 key，`-r 100000`）：**

在引入加锁实现后，使用如下命令进行 pipeline 压测（摘要输出）：

```bash
redis-benchmark -p 26379 -c 50 -n 2000000 -t get,set -P 256 -q -r 100000
```

| 命令 | 吞吐 | p50 |
|------|:----:|:---:|
| SET | 2941255.75 req/s | 2.007 ms |
| GET | 4405313.00 req/s | 1.471 ms |

相较于前面的固定 key 基线（无 `-r`），这组数据在吞吐和延迟上都更保守，符合“加锁 + 更分散 key 空间”带来的额外竞争与路径开销。
...
- `-r 100000` 会把请求分散到更大的 key 空间。它与固定 key 场景的缓存命中和锁竞争模式不同，因此建议单独作为“加锁实现下的随机 key 工况”记录。
...
- [src/redis/aof.test.cpp](src/redis/aof.test.cpp)
- [src/redis/application_context.test.cpp](src/redis/application_context.test.cpp)
- [src/redis/command.test.cpp](src/redis/command.test.cpp)
- [src/redis/command_context.test.cpp](src/redis/command_context.test.cpp)
- [src/redis/commands.test.cpp](src/redis/commands.test.cpp)
...
- [src/redis/reply.test.cpp](src/redis/reply.test.cpp)
...
- [src/redis/sorted_set.test.cpp](src/redis/sorted_set.test.cpp)
...
6. [include/sponge/redis/sorted_set.h](include/sponge/redis/sorted_set.h) 与 [src/redis/sorted_set.cpp](src/redis/sorted_set.cpp)
7. [include/sponge/redis/aof.h](include/sponge/redis/aof.h) 与 [src/redis/aof.cpp](src/redis/aof.cpp)
8. [src/redis/protocol.h](src/redis/protocol.h) 与 [src/redis/protocol.cpp](src/redis/protocol.cpp)
9. [src/redis/db_shard.h](src/redis/db_shard.h) 与 [src/redis/db_shard.cpp](src/redis/db_shard.cpp)
10. [src/redis/application_context.h](src/redis/application_context.h) 与 [src/redis/application_context.cpp](src/redis/application_context.cpp)
11. [src/redis/ttl_manager.h](src/redis/ttl_manager.h) 与 [src/redis/ttl_manager.cpp](src/redis/ttl_manager.cpp)

## 后续可以补的方向

- 支持命令列表与行为说明
- AOF 重放与故障恢复流程说明
- AOF 关闭时的任务完整性保证（`stopping_` 语义讨论）
- 协议层错误处理文档
- 分片策略与线程模型说明
- `SortedSet` 编码升级触发条件与字节阈值文档
- 内存占用与 PMR 资源策略文档
