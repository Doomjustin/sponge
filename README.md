...
补充：针对 SortedSet 的内部结构基准（`benchmark/sorted_set_hybrid_vs_flat_map`）显示，当前查询路径已经足够快：

- `zscore`：hybrid 12.41ms，略优于 flat_map 13.41ms（1.08x）
- `zcount`：hybrid 1.81ms，显著优于 flat_map 705.49ms（389.34x）
- `zrank`：hybrid 1.60ms，显著优于 flat_map 680.64ms（425.93x）
